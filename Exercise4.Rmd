...
<span style="color: red;"> **Comment added later:** in the **Pre-exercise-code of Exercise 4.5**, straight AFTER the line that starts with  **boston_scaled <- read.table**  and ends with **sep=",", header = T)**, you should **ADD ONE LINE:**

*boston_scaled$crime <- factor(boston_scaled$crime, levels = c("low", "med_low", "med_high", "high"))*

Then, add the same line similarly in the Pre-exercise-codes of **Exercise 4.6 and 4.7.**.</span>
...
boston_scaled <- read.table("https://raw.githubusercontent.com/KimmoVehkalahti/Helsinki-Open-Data-Science/master/datasets/boston_scaled.txt",
                            sep=",", header = T)
boston_scaled$crime <- factor(boston_scaled$crime, levels = c("low", "med_low", "med_high", "high")) #NEW LINE
```
...
table(correct = correct_classes, predicted = lda.pred_test$class)
